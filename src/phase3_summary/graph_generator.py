...
class GraphGenerator:
    ...
    def generate_sentiment_balance_chart(
        ...
        """
        Generate Diverging Bar Chart: Positive vs Negative Reviews per Theme.
        Large, clear visualization showing sentiment for all 5 themes.
        """
        try:
            df = data.copy()
            df['sentiment'] = pd.cut(
            ...
            pivot['Total'] = pivot['Negative'] + pivot['Positive'] + pivot['Neutral']
            ...
            max_val = max(pivot['Negative'].max(), pivot['Positive'].max())
            ...
            ax.set_xlabel("Review Count (Negative ← | → Positive)", fontsize=14, color='#64748B', fontweight='500', labelpad=10)
    ...
    def generate_theme_drilldown_chart(
        ...
        try:
            ...
            theme_clusters = []
            for c in clusters:
                if (c.get('theme_id') == theme_id and 
                    c.get('avg_rating', 5) < 3.5 and 
                    c.get('theme_id') != 'UNMAPPED'):
                    theme_clusters.append({
                        'label': c.get('label', 'Unknown Issue'),
                        'size': c.get('size', 0),
                        'avg_rating': c.get('avg_rating', 3)
                    })
            ...
            if not theme_clusters:
                ...
                for c in clusters:
                    if c.get('avg_rating', 5) < 3.5 and c.get('theme_id') != 'UNMAPPED':
                        theme_clusters.append({
                            'label': c.get('label', 'Unknown Issue'),
                            'size': c.get('size', 0),
                            'avg_rating': c.get('avg_rating', 3)
            ...
            ax.set_xlabel("Number of Reviews", fontsize=10, color='#64748B')
