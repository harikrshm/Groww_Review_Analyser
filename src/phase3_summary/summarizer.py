...
class Summarizer:
    ...
    def generate_summary(
        ...
        # 3. Call LLM
        logger.info(f"Generating summary for {week_id}...")
    ...
    def _prepare_clusters_for_prompt(self, clusters_report: Dict) -> List[Dict]:
        """Format cluster data for the prompt, scrubbing PII from quotes."""
        processed_clusters = []
        ...
        for cluster in clusters_report.get("clusters", []):
            ...
            # Fallback: if no quotes found in report, try to use summary/label
            if not quotes:
                # If we can't find quotes, the summarizer might struggle to find "real user quotes".
                # It might hallucinate or use the summary. 
                # In Phase 2 `clusters_report.json` provided earlier, 
                # `representative_ids` were present, but not text.
                # Ideally, we should load `weekly_clusters.json` or have enriched report.
                pass
